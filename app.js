...
function recipeRender(data, reset = false) {
  ...
  data.slice(currentIndex, end).forEach(function (item, i) {
    div.innerHTML += `
      <div class="card" onclick="card(${currentIndex + i})">
        <h1>${item.name}</h1>
        <img src="${item.image}" alt="">
        <p><strong>Difficulty:</strong> ${item.difficulty}</p>
        <p><strong>Rating:</strong> ${item.rating}</p>
        <p><strong>Cuisine:</strong> ${item.cuisine}</p>
        <p><strong>Calories:</strong> ${item.caloriesPerServing}</p>
      </div>
    `;
  });
  ...
  currentIndex = end;
//                                    condition            if       else
  loadMore.style.display = currentIndex >= data.length ? "none" : "block";
}
...
recipes.forEach(function (item) {
  if (!difficultyOpt.includes(item.difficulty)) {
    difficultyOpt.push(item.difficulty);
  }

  if (!cuisineOpt.includes(item.cuisine)) {
    cuisineOpt.push(item.cuisine);
  }
});

//                                              ADD OPTIONS 

difficultyOpt.forEach(function (item) {
  difficulty.innerHTML += `<option value="${item}">${item}</option>`;
});

cuisineOpt.forEach(function (item) {
  cuisine.innerHTML += `<option value="${item}">${item}</option>`;
});
...
function filterData() {
  ...
  let cuisineVal = cuisine.value;

  filter = [];

  recipes.forEach(function (item) {
    let matchDifficulty = difficultyVal === "all" || item.difficulty === difficultyVal;
    let matchCuisine = cuisineVal === "all" || item.cuisine === cuisineVal;
  ...
  if (filter.length === 0) {
    ...
    recipeRender(recipes, true);
    return;
  }

  recipeRender(filter, true);
}

difficulty.addEventListener("change", filterData);
cuisine.addEventListener("change", filterData);
...
loadMore.addEventListener("click", function () {
  if (filter.length > 0) {
    recipeRender(filter, false);
  } else {
    recipeRender(recipes, false);
  }
});
